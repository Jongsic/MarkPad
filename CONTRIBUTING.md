...
Thanks for your interest in MarkPad! This document covers how to build, test,
and submit changes.
...
The Xcode project is **generated** — never edit `MarkPad.xcodeproj` directly.
...
```sh
scripts/build-dmg.sh                                       # from the repo root: Release build → build/MarkPad-<version>.dmg
xcodegen generate                                          # regenerate the project
xcodebuild -scheme MarkPad -destination 'platform=macOS' test   # build & run tests
swiftformat .                                              # format
...
2. `xcodebuild -scheme MarkPad -destination 'platform=macOS' test` passes
...
- **Scope is a feature too.** MarkPad deliberately has no plugins, tabs,
